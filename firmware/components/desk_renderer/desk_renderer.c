...
bool desk_renderer_init(desk_renderer_t *r,void *parent_ptr){
    if(!r||!parent_ptr){return false;}
    memset(r,0,sizeof(*r));lv_obj_t *parent=parent_ptr;
    ...
    lv_obj_t *footer=lv_label_create(root);lv_obj_set_width(footer,152);lv_label_set_long_mode(footer,LV_LABEL_LONG_DOT);lv_obj_set_style_text_color(footer,lv_color_hex(0x3d4a5c),0);lv_obj_set_style_pad_top(footer,4,0);
    r->root=root;r->accent=accent;r->title=title;r->primary=primary;r->secondary=secondary;r->status=status;r->footer=footer;return true;
}
void desk_renderer_set_connectivity(desk_renderer_t *r,const char *text){
    if(!r||!r->footer){return;}
    lv_label_set_text(r->footer,text?text:"");
}
void desk_renderer_render(desk_renderer_t *r,const desk_screen_t *s){
    if(!r||!r->root||!s){return;}
    uint32_t accent=accent_for(s);lv_obj_set_style_bg_color(r->accent,lv_color_hex(accent),0);lv_obj_set_style_bg_opa(r->accent,LV_OPA_COVER,0);lv_label_set_text(r->title,s->title);lv_label_set_text(r->primary,s->primary[0]?s->primary:desk_contract_screen_name(s->type));lv_label_set_text(r->secondary,s->secondary);lv_label_set_text(r->status,s->status);lv_obj_set_style_text_color(r->status,lv_color_hex(accent),0);
    if(s->type==DESK_SCREEN_OVERVIEW){lv_obj_clear_flag(r->footer,LV_OBJ_FLAG_HIDDEN);}else{lv_obj_add_flag(r->footer,LV_OBJ_FLAG_HIDDEN);}
}
